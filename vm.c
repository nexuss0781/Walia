...
#include "effect.h"
#include "hash.h"
#include "vm_operators.h"
#include "vm_magic_names.h"
...
static InterpretResult run() {
    ...
    #define PET_WATCHDOG() sys_watchdog_pulse(core_id)
        ...
        static void* dispatch_table[] = {
            ...
            &&OP_DB_SIMD_FILTER_GT_lbl, &&OP_DB_LOAD_COL_lbl,
...

    {
        ...
        CASE(OP_JUMP)       { PET_WATCHDOG(); frame->ip += GET_BC(READ_INST()); DISPATCH(); }
        CASE(OP_JUMP_IF_FALSE) { PET_WATCHDOG(); Instruction inst = READ_INST(); if (IS_NIL(R(GET_A(inst))) || (IS_BOOL(R(GET_A(inst))) && !AS_BOOL(R(GET_A(inst))))) frame->ip += GET_BC(inst); DISPATCH(); }
        CASE(OP_LOOP)       { PET_WATCHDOG(); frame->ip -= GET_BC(READ_INST()); DISPATCH(); }
