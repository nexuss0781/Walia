...
#include "opcode.h"
#include "vm_db_ops.h"
#include "vm_sys_ops.h"
#include "object.h"
#include "table.h"
#include "hash.h"
#include "sys_types.h"
#include "sys_units_check.h"
...
static void declareLocal(Token name, WaliaSysType type, int* reg, int* offset) {
...
static void emitOp(int op, int a, int b, int c, int line) { writeChunk(currentChunk(), PACK_INSTRUCTION(op, a, b, c), line); }
static void emitOpBC(int op, int a, int bc, int line) { writeChunk(currentChunk(), PACK_INSTRUCTION(op, a, (bc >> 8) & 0xFF, bc & 0xFF), line); }
static int makeConstant(Value value) { return addConstant(currentChunk(), value); }
